-- Fix configure_event_control to not hardcode max_concurrent_per_session = 1
-- Allow it to match max_concurrent_studies so one session can launch multiple studies
...
begin
  ...
  set max_study_budget_cents = p_max_study_budget_cents,
      ...
      -- Allow the same session to hold as many slots as the global limit

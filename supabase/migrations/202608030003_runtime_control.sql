-- Keep the database-enforced control row aligned with validated deployment
-- configuration. The app calls this idempotently before design, launch, report,
-- and recovery work; counters remain owned exclusively by lifecycle RPCs.

create function public.configure_event_control(
...
begin
  ...
  if p_max_concurrent_studies not between 1 and 3 then

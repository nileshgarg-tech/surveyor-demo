...
async function hasStudyAccess(studyId: string): Promise<boolean> {
  try {
    ...
    const [{ data: session }, { data: study }] = await Promise.all([
      ...
      getServiceSupabase().from("studies").select("id").eq("id", studyId).eq("event_session_id", payload.sessionId).maybeSingle(),
    ]);
    return Boolean(session && !session.revoked_at && Date.parse(session.expires_at) > Date.now() && study);
  } catch {
    return false;
  }
}
